...
int start_sniffer(const char *ifname, int promiscuous, int count, const char *format) {
    int fd = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    if (fd < 0) { perror("socket"); return -1; }
    if (bind_to_iface(fd, ifname, promiscuous) < 0) { close(fd); return -1; }
    ...
    while (count != 0) {
        ...
        if (parse_arp(buf, (size_t)n, &info) == 0) {
            ...
            ip_to_str(info.sender_ip, sip);  // NBO -> string
            ip_to_str(info.target_ip, tip);

            print_arp(&info, smac, tmac, sip, tip, format);
            if (count > 0) --count;
        }
    }
